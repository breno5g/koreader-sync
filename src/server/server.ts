...
export function createRequestHandler(
  ...
  return function handleRequest(req: http.IncomingMessage, res: http.ServerResponse) {
    ...
    if (req.method === "POST" && req.url === "/sync") {
      ...
      if (!deviceId) {
        return sendError(res, 401, "Unauthorized: Device ID header missing");
      }
      if (!settings.validDeviceIDs.includes(deviceId)) {
        return sendError(res, 401, "Unauthorized: Invalid Device ID");
      }
      console.log(`[KOReader Sync] ✅ Authorized device (ID: ${deviceId}).`);
      ...
      req.on("end", () => {
        (async () => {
          try {
            ...
            if (!metadata || !metadata.annotations || !metadata.stats) {
              return sendError(
                res,
                400,
            ...
            new Notice(`KOReader Sync: Receiving ${highlightCount} highlights...`);

            await saveHighlightsToFile(app, metadata);
            ...
            console.error("[KOReader Sync] Error processing request:", e);
            if (!res.headersSent) {
              sendError(res, 500, "Internal Server Error: Could not process highlights");
            }
          }
        })();
      });
